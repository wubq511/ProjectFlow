...
- `/workspaces/[workspaceId]` 为跳转路由：有项目则跳第一个项目，无项目则跳新建项目页
- 项目页采用三栏布局：左侧 workspace/project 导航(ProjectSidebar)、中间内容区(ProjectContent)、右侧 Agent 面板(AgentSidebar)
...
| 27 | Code Review Hardening: 正则词边界替换防子串损坏, _validate_references 复用 common.py helpers, rejected-pairs 降级(修复 all-rejected 500), 协商创建 proposal status 校验, 中文约束关键词, docstring 对齐, 阶段名称替代 stage_id, assign 事件加入阶段过滤, 协商输出持久化, _build_user_facing_assignment 分支合并 | 已完成：2026-06-03 |
| 28 | Frontend Redesign Migration: 三栏项目布局(sidebar+content+agent sidebar), 蓝金视觉体系(Instrument Serif+Inter字体), workspace 路由改为跳转项目, AppShell 全高布局, 新组件(project-layout/sidebar/content/agent-sidebar/new-project-dialog/workspace-content/compact-stat/direction-decision-view), 死代码清理(project-dashboard→project-actions) | 已完成：2026-06-04 |
| 29 | UI Critique Fixes: 移除 uppercase eyebrow 残留(4处), 修复 ghost-card(3处), workspace 噪音清理, disabled 状态视觉区分, 方向卡描述优化 | 已完成：2026-06-04 |
| 30 | Workspace Page Critique Fixes: 原生 select→shadcn Select, 搜索/筛选, 成员点击交互, 错误提示层级, StatCard action 链接, 技能标签全展示, uppercase/ghost-card/header 密度优化 | 已完成：2026-06-04 |
| 31 | Onboarding Flow Critique Fixes: 统一 4 步步骤指示器(账号/工作区/资料/项目), 移除 eyebrow 大写残留, 成员资料添加跳过选项, 表单验证标准化(blur+change), 日期选择器添加日历图标, 成功状态添加新建项目入口 | 已完成：2026-06-04 |
