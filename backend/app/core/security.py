...
def require_demo_admin_access(
    ...
    if settings.app_env == "evaluation":
        # Reuse the same evaluator identity + ownership proof as the evidence
        # snapshot. Destructive demo endpoints and read-only evidence reads
        # share the same isolation boundary; both must be bound to the
        # evaluator-owned instance.
        _verify_evaluation_identity(
            x_evaluation_nonce=x_evaluation_nonce,
            x_evaluation_instance_id=x_evaluation_instance_id,
        )
...
def _verify_evaluation_identity(
    *,
    x_evaluation_nonce: str | None,
    x_evaluation_instance_id: str | None,
) -> None:
    """Verify evaluator-owned instance identity and temp-root ownership.

    Reused by both destructive demo endpoints and the read-only evaluation
    evidence snapshot. The evidence snapshot is logically read-only, but it
    exposes viewer-sensitive facts, so it must be bound to the same evaluator
    instance that owns the database.

    Fail-closed: missing nonce/instance/marker/path containment all return 403.
    """
    import os

    if settings.app_env != "evaluation":
        raise HTTPException(status_code=403, detail="评估证据接口仅在 evaluation 环境可用")
...
def require_evaluation_evidence_access(
    authorization: Annotated[
        str | None,
        Header(alias="Authorization"),
    ] = None,
    ...
    """Protect the evaluation evidence snapshot endpoint.

    Combines two independent gates:
    1. Sidecar service token (Authorization: Bearer ...) — same as other
       ``/internal/*`` endpoints.
    2. Evaluator-owned instance identity (nonce + instance ID + ownership
       marker + path containment) — same as the destructive seed endpoint.

    The evidence snapshot is read-only, but it exposes viewer-sensitive facts
    (private conversations, ProjectMemory content). Requiring both gates
    ensures only the evaluator process that owns the isolated database can
    read evidence, and only while the evaluation instance is live.
    """
    require_internal_service_access(authorization=authorization)
    _verify_evaluation_identity(
        x_evaluation_nonce=x_evaluation_nonce,
        x_evaluation_instance_id=x_evaluation_instance_id,
    )
