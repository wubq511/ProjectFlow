...
All MVP and MVP Usable tasks are complete. T41 Agent Runtime, T42 ProjectMemory V1 and T43 Agent Harness V2 P0 are implemented. On 2026-07-13, T44 hardened request deduplication, cache/cost telemetry, model-selection truthfulness, Prompt Kernel/context receipts, Skill effect ceilings and assignment constraint evidence. T45 added private multi-conversation history with legacy team-history migration, viewer-scoped APIs, cursor pagination, URL selection and streaming-safe history switching. On 2026-07-14, settings was moved to the left sidebar footer, the Recent Activity panel was removed, reset demo was relocated into Settings > System, the settings UI was polished with tabbed navigation, slash commands gained a selected-state highlight chip, and the composer was merged with loop-boundary steering (plus steering-poller and mid-stream abort scaffolding in the agent-bridge). The latest deterministic baseline is 852 backend tests passed / 4 skipped, 1142 agent-bridge tests passed across 58 files, and 196 frontend tests passed across 19 files; all lint/typecheck/build gates pass. On 2026-07-15, a 60ms debounce was added to workspace view navigation to coalesce rapid sidebar clicks and eliminate Next.js App Router RSC flight cancellation (`net::ERR_ABORTED`) console noise. The repeated post-T44 production canary passed all frozen routing, outcome, privacy and latency gates; Flash remains the default and Pro remains explicit escalation.
...
Implemented: FastAPI backend with private multi-conversation persistence and service-token-protected internal runtime/tools; T41 typed domain tools and Proposal-Confirm; T42 governed ProjectMemory; T43 durable Agent Harness V2; and T44 request/model/prompt/Skill efficiency hardening. Current verification baseline: 852 backend tests passed / 4 skipped, 1142 agent-bridge tests passed across 58 files, 196 frontend tests passed across 19 files; backend ruff, agent-bridge typecheck/build, and frontend lint/build all pass. See [the post-T44 production canary](docs/T44/post-t44-production-canary-2026-07-13.md) for repeated model evidence and cache/cost interpretation.
