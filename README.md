...
Slice 0 runs the bounded local mock smoke; Issue #95 supplies ProjectFlow-aware deterministic hard-domain grading via `smoke-v2`; merged Issue #96 supplies multi-turn, Skill, Runtime, reliability, paired comparison and the Slice 1 exit gate. The deterministic Slice 1 closure evidence is the mock `full` run plus `verify` and `exit-gate`; paid-model quality/cost calibration is a later optional evidence layer, not a prerequisite. Coding Agent cost is reported separately and never counts against the ProjectFlow Agent cap. Issue #97 is the next slice and owns evidence-graded RCA and Repair Packets.
