...
Slice 0 runs only the bounded local mock smoke; Issue #95 adds the ProjectFlow-aware deterministic foundation via `smoke-v2`, including public Proposal confirm/reject; Issue #96 (branch `glm/t46-96-conversation-runtime-reliability`, not yet merged) adds the multi-turn user controller, simulator integrity, attempt ledger, Skill 8 dimensions, Runtime 11 fault classes, `demo`/`smoke`/`smoke-v2`/`full` presets, candidate/baseline paired execution, reliability statistics, operational metrics, and a 6-condition Slice 1 exit gate. The deterministic Slice 1 closure evidence is the mock `full` run plus `verify` and `exit-gate`; paid-model quality/cost calibration is a later optional evidence layer, not a prerequisite. Paid models fail closed until a frozen price table and pre-call worst-case estimate exist. Coding Agent cost is reported separately and never counts against the ProjectFlow Agent cap.
