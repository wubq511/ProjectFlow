...
- Current verification baseline: backend pytest 519 passing, 4 skipped; agent-bridge 540 tests passing with typecheck/build passing; frontend 46 tests passing; changed backend files pass ruff; frontend lint/build/audit baseline remains from the Phase 41 verification pass.
- As of 2026-07-14, the implementation has progressed through T41–T45 and an additional UX/Runtime polish pass. The latest deterministic baseline is 852 backend tests passed / 4 skipped, 1142 agent-bridge tests passed across 58 files, and 191 frontend tests passed across 18 files; all lint/typecheck/build gates pass. See `docs/handoff.md` for the detailed 2026-07-14 handoff.

T41 Agent Runtime update: on 2026-07-04, the target Agent runtime architecture was confirmed separately in `docs/T41/`. Through 2026-07-06, runtime/tool slices S3/S5/S6/S7/S8/S9/S10/S11/S12/S13/S14/S15/S16 are implemented: `generate_stage_plan_proposal`, `analyze_checkins_and_risks`, `recommend_assignment`, `generate_replan_proposal`, `generate_direction_card_proposal`, `generate_task_breakdown_proposal`, `create_risk`, and `create_checkin` are registered as typed internal tools. Agent-inferred check-in task status changes now create pending replan proposals instead of directly writing `Task.status`; Risk, ActionCard, direct risk, and direct check-in records are advisory writes; AssignmentProposal creation does not write `Task.owner_user_id`; proposal confirmation/rejection remains on the public proposal API. This TECH-DESIGN remains the MVP/current-implementation design. For new Agent runtime work, use the T41 docs, `CONTEXT.md`, and `docs/adr/` as the source of truth.
