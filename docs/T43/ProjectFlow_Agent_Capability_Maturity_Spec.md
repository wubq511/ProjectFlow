...
> 状态：Working-tree implementation under validation
> 日期：2026-07-12
> 目标：在不突破 ProjectFlow 业务安全边界的前提下，把现有领域 Agent 从“能调用工具的业务自动化”升级为“可规划、可恢复、可验证、可扩展、可持续评测的成熟 Agent Harness”。

## Executive Conclusion

ProjectFlow 当前并不是缺少 Agent 底座。它已经具备多轮模型循环、12 个 typed tools、工具 manifest、policy gate、预算与取消、流式事件、trace、Proposal-Confirm、ProjectMemory、6+1 个 Skills 和较强的隐私约束。这些能力使它明显高于普通的单轮聊天机器人。
...
| 验证面 | 当前结果 |
|---|---|
| Agent Bridge tests | 955/955 passed（54 files） |
| Agent Bridge typecheck/build | passed / passed |
| Backend tests | 754 passed / 4 skipped |
| Backend ruff | passed |
| Frontend tests | 117/117 passed（14 files） |
| Frontend lint/build | passed / passed |
| Patch hygiene | `git diff --check` passed |

### 尚未满足的发布证明

- 还没有统一公开入口的真实场景基线，无法给出 routing accuracy、outcome pass rate、P95 latency、token 和 cost 的可信数值。
- 还没有 effect approval 的完整执行门禁和前端审批 UX；当前只保留 Proposal-Confirm 与控制面状态基础。
- 前端只有 resume/steering API 与类型，没有完整 Agent 控制 UI。
- large-result 只有 bounded `ToolResourceRef`，没有 durable resource pagination/fetch contract。
- trajectory exporter、跨模型 conformance job、真实模型 canary 尚未实现。
- semantic LLM judge 仍按设计延后；必须先用真实场景证明确定性 verifier 存在不可接受的语义缺口，再引入额外成本和非确定性。

因此，当前结论是：**Agent Harness V2 的 P0 核心已形成并通过仓库级回归，但 T43 的产品成熟度和发布 Exit Gates 仍是部分满足，不能宣称“已达到主流 Agent 基本水平”。**
...
| 工具设计 | 中强 | 12 个领域工具边界合理，但注册表可静默覆盖，缺少能力发现和组合层 | schema/version 校验、动态暴露、搜索和 capability packs |
...
### First tracer implementation evidence (working tree, 2026-07-12)

首个 tracer 已在工作树实现并通过独立对抗性复查，但尚未提交，不能视为整个 T43 或 Phase 1 已完成：
...
### Second slice: Shared request preparation + Outcome Contract + Prompt kernel (working tree, 2026-07-12)
...
### Phase 2: Context Engine + Skills V2 (working tree, 2026-07-12)
...
### Phase 3: RunPlan + WorkState + Deterministic Verifier (working tree, 2026-07-12)
...
### Phase 4: Tool Executor V2 (working tree, 2026-07-12)
...
### Phase 5: Checkpoint + Resume + Steering (working tree, 2026-07-12)
...
### Phase 6: Operational Hardening (working tree, 2026-07-12)
