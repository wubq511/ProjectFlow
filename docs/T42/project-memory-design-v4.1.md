...
V1 不做通用自我约束框架。唯一窄例外是已注入 `member_constraint` 的分工请求：sidecar 在输出边界执行独立审查、一次修复、复审和确定性成员检查，失败时保守降级；它不改变记忆事实源，也不扩展到方向、阶段或任意策略裁决。
...
→ 若 used_memory_types 包含 member_constraint 且请求涉及分工，输出守卫在 SSE 前缓冲并审查最终文本
→ 守卫通过、修复通过或保守 fallback 后，记录 output_guard_status / output_guard_model_calls
```
