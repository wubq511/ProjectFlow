...
**Verification:** the light release gate passed Agent Bridge 2255/2255 across 94 files, typecheck/build, 137/137 targeted Golden Core/hard-grader checks, `golden-core freeze` + `verify`, inherited `full` mock 16/16 and mock `calibrate`. The real 52-scenario public-seam mock run published an integrity-verified regression artifact with 30/52 passing at `$0.00` SUT cost. Release-gate fixes removed non-existent Skill IDs, aligned allowlists with ToolManifest effect types and retained fail-closed prohibited-commit semantics. The remaining failures are deliberately preserved for the final cross-slice adversarial review and repair alignment; they were not hidden by weakening graders. Backend/frontend were not rerun because #99 changed neither surface.
...
`calibrate` exits `0` only when the calibration exit gate passes; `1` when fail-safe, bias, acceptance or unresolved-conflict evidence fails (the immutable artifact is still published); `2` on infrastructure failure; `3` on validation failure; `4` only on budget exhaustion with partial evidence. `promote-standard` exits `0` when the approval record is built and the candidate is approved; `1` when the candidate is not in `approved` status, fingerprints do not match, or unresolved conflicts remain; `3` validation. The approval record NEVER claims cryptographic identity authentication for Robert — it is repository governance with reviewable Git history.

**Key files:** `agent-bridge/src/evaluation/lab/calibration-contract.ts`, `agent-bridge/src/evaluation/lab/standards-registry.ts`, `agent-bridge/src/evaluation/lab/standard-conflicts.ts`, `agent-bridge/src/evaluation/lab/semantic-judge.ts`, `agent-bridge/src/evaluation/lab/judge-bias.ts`, `agent-bridge/src/evaluation/lab/calibration-runner.ts`, plus V5 extensions to `presets.ts`, `artifact-store.ts`, `cli.ts`, `validation.ts`. Test files: `t46-5-calibration-contract.test.ts`, `t46-5-standards-registry.test.ts`, `t46-5-standard-conflicts.test.ts`, `t46-5-semantic-judge.test.ts`, `t46-5-judge-bias.test.ts`, `t46-5-calibration-runner.test.ts`, `t46-5-mutation.test.ts`, `t46-5-governance.test.ts` (225 tests total).

**Verification:** 8 t46-5 files with 225 tests cover the V5 contract, registries, conflicts, semantic Judge, bias metrics, calibration runner, mutation and governance. The light release gate passed Agent Bridge 2170/2170 plus typecheck/build, `validate --preset calibrate`, `conflict-catalog`, and a real deterministic `calibrate review98_calibrate_gate2 --json` run with all six bias metrics populated and `passed: true`. The gate repaired four real-entry defects: missing mock Judge inputs, incorrect anchor-repeat/order grouping, promotion eligibility contradicting fail-safe evidence, and non-budget failures returning exit `4`. Backend/frontend were not rerun in this gate because #98 changed neither surface; their previously recorded baselines remain unchanged. Full cross-slice adversarial review is deferred until all T46 tickets are complete per Robert's instruction.

### 2026-07-20 — T46 Issue #97 Slice 2 Evidence-Graded RCA & Repair Packets

GitHub Issue #97 was merged into `main` at `3e09596` and closed on 2026-07-20. It is the Slice 2 diagnosis & repair surface: it turns trusted evaluation failures into (1) evidence-graded diagnoses with frozen causal statuses, (2) evaluator-owned single-variable counterfactuals and fault injection evidence, (3) a known-fault RCA benchmark with anti-gaming metrics, (4) evidence-bound issue clustering, (5) immutable Repair Packets with versioned schema and stale detection, and (6) copy-ready Coding Agent prompts that forbid weakening frozen standards. Issue #98 subsequently added governed calibration and semantic standards.

Frozen diagnosis statuses are `observed_failure | localized_hypothesis | intervention_supported | fault_injection_confirmed | unresolved`. `intervention_supported` cannot be promoted to `fault_injection_confirmed`; that promotion requires a fresh fault injection from `localized_hypothesis`. Eight evaluator-owned fault profile categories are required: routing, context, skill, tool_schema_or_result, policy_or_effect_boundary, privacy_or_visibility, proposal_evidence, terminal_events. The RCA benchmark uses three sample classes — `correct_attribution`, `confusable_neighbour`, `unresolved_or_insufficient` — with confidence ceilings enforced per class. The five benchmark gates are `top1Accuracy ≥ 0.5`, `falseAttributionRate ≤ 0.3`, `confidenceCalibration ≥ 0.7`, `evidenceCompleteness ≥ 0.7`, `top3Recall − top1Accuracy ≤ 0.4`. Anti-gaming: `correct_attribution` samples only count toward `top3Recall` when their `top1` prediction is also correct.

Repair Packets use `REPAIR_PACKET_SCHEMA_VERSION = 1`. A `fix` packet requires direct component evidence OR `intervention_supported` OR `fault_injection_confirmed`, PLUS a falsifiable acceptance test, protected boundaries, verification commands, and a valid code fingerprint; otherwise the packet is `investigation`. Packets are scrubbed of secrets (api keys/tokens/passwords/bearer), absolute temp paths, raw hidden-fact markers (`__hidden__`/`__oracle__`/`__expected_cause__`), and model hidden reasoning (`<think>`/`<reasoning>`/`<hidden>`). Stale detection fails closed when the commit or worktree SHA-256 differs. Candidate regressions are marked `candidate`/`unapproved` and cannot be auto-promoted. The Coding Agent prompt forbids modifying `.env`, API keys, internal service tokens, frozen standards, graders, thresholds, P0 cases, and privacy/authority boundaries; forbids auto push/merge/Issue close; and refuses to execute on a stale packet.

**Operator/Coding Agent commands:**

```bash
# Diagnose failed observations, generate packets, verify RCA, validate fault catalog.
...
For a fully-passing mock `full` run, `diagnose` returns `diagnosis_skipped` (exit `0`) and `repair-packet` returns `repair_packets_empty` (exit `0`) — both are the expected fail-open path. `rca-benchmark` runs the full known-fault suite (12 mock samples) and returns `passed: true` only when all five gates are satisfied. `fault-catalog` validates all 8 fault-profile categories and returns `{"event":"fault_catalog_valid","complete":true}`.

**Key files:** `agent-bridge/src/evaluation/lab/diagnosis-contract.ts`, `agent-bridge/src/evaluation/lab/diagnosis-runner.ts`, `agent-bridge/src/evaluation/lab/fault-profiles.ts`, `agent-bridge/src/evaluation/lab/counterfactual.ts`, `agent-bridge/src/evaluation/lab/earliest-divergence.ts`, `agent-bridge/src/evaluation/lab/rca-benchmark.ts`, `agent-bridge/src/evaluation/lab/issue-clustering.ts`, `agent-bridge/src/evaluation/lab/repair-packet.ts`, `agent-bridge/src/evaluation/lab/repair-prompt.ts`, `agent-bridge/src/evaluation/lab/cli.ts`, and the test files `t46-4-diagnosis-contract.test.ts`, `t46-4-fault-profiles.test.ts`, `t46-4-earliest-divergence.test.ts`, `t46-4-counterfactual.test.ts`, `t46-4-rca-benchmark.test.ts`, `t46-4-issue-clustering.test.ts`, `t46-4-repair-packet.test.ts`, `t46-4-repair-prompt.test.ts`, `t46-4-diagnosis-runner.test.ts`, `t46-4-mutation.test.ts`.

**Verification:** 10 new test files with 317 tests cover the contract, fault profiles, earliest divergence, counterfactuals, RCA benchmark with anti-gaming, issue clustering, repair packet invariants, repair prompt invariants, end-to-end diagnosis pipeline, and 46 mutation tests across all declared gates including symmetry coverage supplements. Real CLI path verified: `diagnose` → `diagnosis_skipped` (no failures), `rca-benchmark` → 12 samples passed all 5 gates, `exit-gate` → 6/6 conditions passed, `fault-catalog` → `complete: true`, `repair-packet` → `repair_packets_empty` (no diagnosis). Paid-model execution is not required for #97 closure; Proposal-Confirm, FastAPI source of truth, sidecar not reading/writing business DB, public HTTP/SSE seam, evaluator-owned isolation, immutable checkpoint/SHA-256, and Slice 0/1 compatibility boundaries remain unchanged.

### 2026-07-20 — T46 Issue #96 Slice 1 Multi-Turn / Skill / Runtime / Reliability

GitHub Issue #96 was independently hardened, merged into `main` at `50c0f77`, and closed on 2026-07-20. It completes Slice 1's remaining acceptance surface on top of #95's hard-domain foundation: deterministic multi-turn user controller, simulator integrity, attempt ledger, Skill evaluation (8 dimensions), Runtime fault matrix (11 fault classes), demo/smoke/smoke-v2/full presets with P0 scenario IDs and budget caps, a paired runner that creates separate detached worktrees and isolated backend/sidecar/database/temp/artifact resources, reliability statistics that distinguish one-shot observations from true repeat groups, operational metrics with SUT/evaluator/coding-agent cost separation, and a 6-condition fail-closed Slice 1 exit gate. Proposal-Confirm, FastAPI as source of truth, sidecar not reading/writing business DB, public HTTP/SSE seam, evaluator-owned isolation, immutable checkpoint/SHA-256, paid-model fail-closed, and Slice 0/1 compatibility boundaries remain unchanged. Its successor Issue #97 is also merged and closed.
...
**Verification:** backend 890 passed / 4 skipped plus Ruff; agent-bridge 1628 passed across 75 files plus typecheck/build; frontend 333 passed / 6 skipped across 26 files plus lint/build. The hardened mock `full` run passes 16/16 required scenarios and its six-condition `exit-gate` passes; `demo` passes 5/5. `verify` checks the immutable result graph. `reliability` correctly returns insufficient evidence for a single one-shot run instead of inventing repeat-run significance. `compare` executes both refs in separate detached worktrees and isolated runtime/state/artifact resources; unresolved model identity is reported as possible drift rather than silently accepted. Paid-model execution is not required to close #96 and remains fail-closed until pricing and pre-call bounds are frozen.

### 2026-07-19 — T46 Issue #95 Slice 1 Hard-Domain Foundation

GitHub Issue #95 was merged into `main` on 2026-07-19 (`ac3e68d`) and closed. It is the Slice 1 hard-domain foundation; Issue #96 subsequently completed multi-turn, Skill/Runtime reliability and baseline/candidate reporting. #95 adds a scoped read-only evidence seam, true event/tool Milestone DAGs, 15 deterministic hard graders, full-oracle Reference Programs, mutation validation, evaluator-only hidden-field commitments, and an isolated `smoke-v2` that exercises answer-only plus public Proposal confirm/reject.
...
Latest deterministic verification baseline after Issue #95 adversarial remediation and `main` integration on 2026-07-19:
...
T41-T45 and T46 Issues #94-#99 are merged and closed. Issue #100 is next and owns the portable evidence-backed showcase, loopback read-only viewer, bounded two-to-five-minute preview, retention planning, Agent-first acceptance and T46 closeout. The final cross-slice adversarial review and repair alignment remains intentionally deferred until all T46 tickets are implemented. Paid evaluation stays fail-closed until pricing and pre-call bounds are frozen.
