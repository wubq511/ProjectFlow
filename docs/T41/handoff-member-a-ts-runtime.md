...
**已完成（2026-07-05，merged through PR #66）。**
...
- Idempotency：同 key 复用同一个 AssignmentProposal；不同 key 对同一任务重复创建会被业务校验拒绝
- Route contract：sidecar path 使用 `assignment-recommendation`，envelope `tool_name` 可保留 model-facing `recommend_assignment`

**验收标准：**
- [x] tool manifest 和 endpoint 实现完成
- [x] 创建 AssignmentProposal 不写 Task.owner_user_id
- [x] idempotency 测试通过
- [x] 现有 assignment flow 测试继续通过

**合并修正：**
- 修复 path endpoint name 与 model-facing tool name 不一致导致的 backend 404。
- 修复同 idempotency key 重试被误判为 duplicate proposal 的问题。
- Validation error observation 改为中文业务错误，不暴露 raw task/user ID。

### Advisory Write Tools (create_risk / create_checkin)

**已实现（2026-07-06，merged through PR #69 and review fixes）。**

两个 advisory write 工具，允许 Agent 创建 advisory 记录：

| 工具 | riskCategory | proposalConfirmRequired | effectType |
|------|-------------|------------------------|------------|
| `create_risk` | advisory_write | false | advisory_record_persisted |
| `create_checkin` | advisory_write | false | advisory_record_persisted |

- `create_risk`：创建 Risk 记录，直接写入，无需确认；校验 stage/task 必须属于目标 project，且 task/stage 组合必须一致
- `create_checkin`：创建 CheckInCycle + CheckInResponse，直接写入；必须提供明确的 workspace member `user_id`，不允许 fallback 编造成员
- `update_stage_progress` 未进入 active tool registry：当前没有合法的 Stage progress commit model，不能用 `proposal_type="plan"` 包装不匹配 payload
- `submit_tool_result` 未进入 active tool registry：confirm/reject proposal 仍由公开 proposal API 承担，不作为 internal agent tool

**验收标准：**
- [x] tool manifest 和 endpoint 实现完成
- [x] create_risk/create_checkin 直接写入 advisory 记录
- [x] create_checkin 必须显式绑定 workspace member
- [x] create_risk/create_checkin 拒绝跨 project/stage/task 的非法引用
- [x] side_effect_status 正确区分
- [x] 幂等处理：同一 idempotency_key 返回缓存结果
- [x] retry.maxAttempts=1（advisory 工具不重试）
- [x] 当前仓库基线：540 sidecar tests（18 个文件）+ 385 backend tests 通过
...
**验收标准：**
- [x] stream 接收和展示（useAgentStream.ts, StreamingText.tsx）
- [x] proposal confirm/reject（agent-proposal-panel.tsx）
- [x] advisory dismiss/resolve（action-card.tsx, risk-card.tsx）
- [x] AssignmentProposal response/finalize（assignment-flow-panel.tsx）
- [x] Run state 展示 + cancel 按钮（ChatComposer with onStop）
- [x] `../scripts/npm run test` / `lint` / `build` 通过（2026-07-06 验证：46 tests pass，0 errors，build pass）
