...
- [ ] sidecar 不持有 DB 凭据
- [ ] Context Builder 正确组装 stable prefix + dynamic suffix
- [ ] Model router 支持多 provider 配置
...
**验收标准：**
- [ ] skill-index/loader/selector 实现完成
- [ ] 6 个 SKILL.md 编写完成
- [ ] skill 匹配测试通过
- [ ] allowed-tools 约束测试通过
...
**验收标准：**
- [ ] `trace_include_sensitive_data` config 实现
- [ ] 默认模式 trace 不含 sensitive data
- [ ] debug 模式 raw payload 进入受控存储
