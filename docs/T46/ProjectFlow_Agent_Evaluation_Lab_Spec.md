...
> 状态：实施前对抗审查已完成；作为 umbrella spec，Ready for ticket decomposition，禁止一次性整体实施
> 日期：2026-07-16
> Issue：[#93](https://github.com/wubq511/ProjectFlow/issues/93)
> 对抗审查：[`ProjectFlow_Agent_Evaluation_Lab_Pre_Implementation_Adversarial_Review.md`](ProjectFlow_Agent_Evaluation_Lab_Pre_Implementation_Adversarial_Review.md)
...
- 失败时，系统能区分现象、最早偏离点、局部化假设、干预支持结论和故障注入已知原因，不把相关性自动写成根因；
...
这九个能力描述最终架构，不代表一个实现批次。Issue #93 是 umbrella spec；实施前必须拆成有独立验收和回滚边界的 tracer-bullet tickets。任何 Coding Agent 都不得把 #93 当成一次性重构任务。
...
Scenario Contract 必须先独立声明目标状态和不变量，Reference Program 后写且只证明“该 fixture 可到达目标、harness 能观察目标”。不得从 Reference Program 或被测 Agent 的实际输出反推 oracle，否则会形成循环自证。
...
65. As a ProjectFlow developer, I want observed trial pass rate, empirical all-k reliability, pass@k and all-invariant-pass reported with explicit definitions, so that capability and reliability are not conflated.
...
101. As a repository maintainer, I want every destructive fixture action bound to an evaluator-created temporary root and nonce, so that a path typo or development mode cannot erase real data.
102. As Robert, I want every paid preset to state whether cost is provider-reported, estimated from versioned pricing or unknown, so that a dollar cap is never presented as enforced when it is not measurable.
103. As a ProjectFlow developer, I want candidate and baseline commits run in separate isolated worktrees and runtime pairs, so that historical comparison is a real paired experiment rather than a stored-chart comparison.
104. As a Coding Agent, I want diagnosis to use explicit evidence levels from observed failure to fault-injection-confirmed cause, so that a suggested code surface is not mistaken for a proven root cause.
105. As a ProjectFlow developer, I want invalid simulator turns classified as `simulator_error` and excluded from Agent scores, so that evaluator defects do not become product regressions.
106. As Robert, I want conflicting specifications, ADRs, schemas and observed code behavior surfaced as `standard_conflict`, so that the evaluator cannot silently choose the implementation it is supposed to test as truth.
107. As a showcase viewer, I want a portable redacted evidence bundle, while a local technical reviewer can use a loopback-only read viewer, so that presentation and private diagnosis do not share one unsafe surface.
108. As a repository maintainer, I want artifact retention and cleanup eligibility reported without automatic destructive deletion, so that evidence does not grow forever or disappear without review.
109. As a Coding Agent, I want #93 decomposed into independently verifiable tracer-bullet tickets before implementation, so that partial completion cannot leave an untestable evaluation platform.
110. As a repository maintainer, I want the evaluator to use the repository-pinned Node toolchain without forcing a root monorepo migration, so that evaluation work does not restructure unrelated packages.
...
- Use a dedicated logical evaluation runtime boundary and the repository-pinned Node toolchain. A standalone package is allowed, but T46 must not require converting the repository into a root npm workspace or migrating the existing backend, agent-bridge or frontend package structure. The system under test must not import scenario expectations, hidden facts, graders, anchors or reference programs.
...
- Use a file-backed local artifact store for V1. Complete artifacts remain local and ignored by Git; approved redacted baselines and showcase bundles may be committed. Schema readers fail closed on unsupported future versions and use explicit migration for supported older versions.
- Use stable pseudonyms within each redacted bundle so repeated references remain diagnosable without exposing raw IDs. Pseudonym salts are bundle-scoped, not global, to prevent correlation across bundles. Raw evidence uses restricted local permissions and never contains secrets.
- Provision an evaluator-owned temporary root containing SQLite database, upload directory, config copies, artifact directory, ephemeral internal token, evaluation nonce and a dedicated backend/sidecar process pair per worker. Autonomous presets may not accept an arbitrary caller-provided database URL.
- Require positive environment ownership proof before any seed or reset: `APP_ENV=evaluation`, the resolved database and upload paths are inside the evaluator-owned temporary root, the process presents the matching nonce/token, and both services report the expected evaluation instance identity. A blacklist of known development or production paths is defense in depth only, never the primary safety check.
- Add a normalized, versioned evaluation evidence snapshot through an authenticated, evaluation-only read seam. It exposes only grader-required state, event, proposal, memory and metric facts; it is not a raw database dump and cannot mutate state. Public HTTP/SSE remains the behavior seam.
...
- Use deterministic fixture builders and seeds. The Scenario Contract defines goal state and invariants independently; a separately authored Reference Program only verifies reachability and harness observability. It cannot generate or overwrite the oracle from its own behavior, and the Agent is not required to reproduce its exact trajectory.
...
- Build a Golden Core coverage matrix with normal, negative, boundary, insufficient-information, conflict, goal-switching, adversarial and multi-turn classes. Approximately 50–64 high-quality canonical Scenario Contracts remain the maturity target, not the first implementation gate. Each delivery slice adds only the cases needed to prove its contracts; coverage completeness and risk coverage matter more than raw count.
...
- Implement a deterministic user controller for multi-turn scenarios. The controller owns goal, hidden facts, refusal conditions and allowed actions; an optional LLM only realizes the allowed action in natural language. Simulator compliance is graded separately. Invalid or leaking simulator turns produce `simulator_error`, are excluded from Agent capability scores and may be retried only within the frozen simulator budget.
...
- Blind candidate model identity where possible, use a different Judge family from the candidate where practical, randomize pairwise ordering and record disagreement. If a suitable independent Judge is unavailable, semantic results remain `needs_review`; the system never silently substitutes an uncalibrated same-family Judge as a hard gate. Semantic scores default to soft metrics until anchors and stability evidence justify a hard threshold.
...
- Report observed trial pass rate, empirical all-`k` reliability groups, all-invariant-pass, per-scenario failure, confidence intervals, latency percentiles, token, tool, retry, error and timeout metrics. `pass@k` is reserved for “at least one of k succeeds”; `pass^k` may be shown only as an explicitly modeled estimate with its independence assumptions, never as a substitute for observed all-`k` results. Smoke and demo make no statistical significance claim.
- A release regression claim requires a paired experiment: baseline commit and candidate fingerprint run in separate isolated worktrees and backend/sidecar pairs with the same scenario version, seed, resolved model configuration, standards, Judge and repetition policy, preferably in the same time window. Stored historical charts are descriptive only. If provider/model version cannot be pinned or resolved, mark `model_drift_possible`. Differential improvement cannot replace absolute invariants.
- Separate infrastructure/provider retries from Agent-internal retries. An infrastructure retry creates a new observation attempt and does not retroactively convert a failed or incomplete Agent attempt into a success.
...
- Treat the earliest divergence as a root-cause hypothesis. Diagnosis uses evidence states `observed_failure`, `localized_hypothesis`, `intervention_supported`, `fault_injection_confirmed` and `unresolved`. A deterministic grader confirms the symptom, not automatically the implementation cause; single-variable interventions can support localization but multi-factor alternatives remain recorded.
- Add a fault-injection benchmark with known failures for routing, context, tool schema/result, effect boundary, privacy, terminal events, visibility and proposal evidence. Injection uses isolated adapters or evaluator-owned fault profiles and never edits the user's worktree or exposes model-callable fault controls. Measure RCA top-1 accuracy, top-3 recall, false attribution and evidence completeness.
...
- Generate immutable Repair Packets with schema version, packet type, severity, confidence, run, code/worktree fingerprint, affected components, observed and expected behavior, reproduction, evidence, root-cause status, suggested scope, protected boundaries, non-goals, acceptance criteria, verification and candidate regression. Candidate code surfaces are labeled as hypotheses unless direct component evidence supports them.
- Use `fix` Repair Packets only when a cause is localized by direct component evidence, intervention support or known fault injection and the acceptance test can falsify the proposed fix. Use `investigation` packets for unresolved or multi-factor hypotheses and require bounded diagnostic experiments before implementation.
...
- Keep calibration separate from normal evaluation. A calibration run produces candidate standards, evidence, anchors, mutation results, Judge bias/disagreement metrics, cost and a reviewable diff. Failed or unapproved calibration leaves active standards unchanged. Conflicts among canonical docs, ADRs, schemas, public behavior and code are recorded as `standard_conflict` and block promotion until explicitly resolved; observed implementation behavior never becomes truth merely because it exists.
- Active standards are checked-in, versioned files. Promotion requires an explicit Robert instruction and a reviewable Git diff/approval record; Coding Agents and Judges cannot self-promote thresholds, rubrics or cases. This is repository governance, not a claim that the local evaluator can cryptographically authenticate Robert's identity.
...
- Serve one immutable result graph through two explicitly different presentation surfaces: a portable committed redacted showcase bundle for external demonstration, and a loopback-only local read viewer for private technical drill-down and live progress. The local viewer has no run, cancel, reset, promotion or repair mutation controls; Coding Agents and the CLI own those actions.
- Both views display release verdict, hard gates, capability matrix, reliability, cost, representative evidence chains, issue clusters, Repair Packets, regression closure, coverage and excluded/error counts. A two-to-five-minute live run is launched through the CLI and atomically publishes a separately labeled preview bundle; it never overwrites an accepted baseline.
- Do not recompute grades in either presentation surface. Verify artifact schema, hashes and provenance before rendering. Committed bundles contain no absolute local paths, raw traces, hidden prompts or cross-bundle stable identifiers.
...
- Record cost source as `provider_reported`, `versioned_price_estimate` or `unknown`. Dollar ceilings are enforceable only for reported cost or a frozen pricing table; when SUT cost is unknown, paid presets refuse to start without an explicit override and still enforce token, request, observation and wall-time ceilings. Reports never display an estimated/unknown cap as provider-confirmed spend.
- Cap ProjectFlow Agent spend per run at $0.10 for smoke, $1 for full and $3 for calibrate. Demo is contained within the smoke ceiling. These caps exclude Coding Agent and evaluator-model cost. Every Judge/simulator run also requires a separate manifest ceiling for calls, tokens, time and, when measurable, dollars; it may not run unbounded. Budget exhaustion stops new observations, preserves completed evidence and yields a partial result.
- Produce a retention plan that reports artifact size, age, status and cleanup eligibility. V1 does not automatically delete artifacts: failed/`needs_review` evidence, Repair Packets, promoted baselines and referenced showcase bundles are preserved; any cleanup is explicit and previewable.
...
Issue #93 defines architecture and acceptance direction. Before implementation it must be decomposed into the following dependency-ordered tracer bullets; each slice must be independently usable, tested and reversible.

### Slice 0 — Minimum Trustworthy Loop

- Deliver versioned Scenario/Manifest/Observation/Grade/Artifact contracts, evaluator-owned process isolation, one public HTTP/SSE scenario, one deterministic state/authority grader, resumable artifact writing, bounded CLI JSON output and a minimal repository-local Agent Skill.
- Reuse one existing T43 release scenario rather than creating a new suite first.
- Exit gate: a Coding Agent can discover and run one zero/low-cost smoke observation; positive ownership proof prevents any reset outside the evaluator root; the result can be reproduced and inspected without importing SUT internals.

### Slice 1 — ProjectFlow Hard-Domain Evaluation

- Add the normalized evaluation evidence snapshot, state constraints, Milestone DAG, Proposal-Confirm, privacy/visibility, read-only purity, Skill and controlled multi-turn graders.
- Grow the Golden Core by risk and capability coverage. Do not pursue the 50–64 maturity target until each added scenario has independent goal provenance, a solvable fixture and grader mutation evidence.
- Exit gate: all declared P0 mutations are detected, reference paths produce zero false hard failures, hidden-field leakage tests pass and hard-domain verdicts require no semantic Judge.

### Slice 2 — Diagnosis and Repair Handoff

- Add evidence-state diagnosis, isolated fault injection, bounded counterfactual experiments, issue clustering, immutable Repair Packets, stale detection and candidate regression staging.
- Exit gate: the known-fault benchmark reports RCA accuracy/recall/false attribution; every `fix` packet meets the stronger causal-evidence rule; unresolved cases produce `investigation` packets rather than speculative edits.
...
- Add fail-closed tests proving seed/reset requires `APP_ENV=evaluation`, an evaluator-owned resolved path, matching instance identity and nonce/token. Test symlink escapes, relative paths, caller-supplied database URLs, reused nonces and accidental development/production targets; a path blacklist alone must not make a run safe.
...
- Add evidence-snapshot contract tests proving it is authenticated, evaluation-only, read-only, normalized, sufficient for declared graders and does not expose arbitrary tables or private data outside the scenario viewer.
...
- Add reference-program tests proving every stateful fixture can reach the independently declared goal state and every reference program satisfies hard gates. Add provenance tests proving the goal oracle is unchanged when the reference implementation changes.
- Add grader mutation tests that delete required milestones, add forbidden writes, reorder strict actions, introduce privacy leaks, cross viewer identity, duplicate terminal events and unknown side effects; every declared mutation must be detected before that grader is released, while reference paths must have zero false hard failures.
...
- Add controlled User Simulator tests proving it never reveals hidden facts, changes the user goal, accepts a forbidden proposal or helps the Agent complete its own task. Invalid simulator episodes must become `simulator_error`, remain outside Agent denominators and consume only their bounded retry budget.
...
- Add counterfactual tests proving only one factor changes between paired runs, fault controls exist only in evaluator-owned isolation and causal status never advances beyond the available evidence.
...
- Add calibration lifecycle tests proving candidate standards are isolated, standard diffs are complete, `standard_conflict` blocks promotion, failed calibration leaves active standards unchanged and only an explicit reviewed diff becomes active.
- Add cost-ledger tests proving ProjectFlow Agent, evaluator models and external Coding Agent cost are not mixed, and each cap applies only to the intended bucket. Cover reported, estimated and unknown cost sources and refusal/override behavior.
- Add budget-stop tests proving new observations stop at SUT or evaluator ceilings while completed observations and partial reports remain usable. Verify call/token/time ceilings remain effective when dollar cost is unknown.
- Add statistics tests for observed trial pass rate, empirical all-`k` reliability, `pass@k`, explicitly modeled `pass^k`, confidence intervals, paired comparison, small-sample insufficient-evidence and missing telemetry coverage.
- Add paired-baseline tests proving baseline and candidate use distinct worktrees/runtime pairs, aligned manifests and separate artifacts; unresolved provider versions must set `model_drift_possible`.
- Add artifact tests for immutable manifests, hashes, atomic publish, resume, migration, unsupported future schema rejection, local permissions, bundle-scoped pseudonyms, retention planning and malformed/stale result rejection.
...
- Add presentation tests proving the portable bundle and loopback viewer render the same verdict as the structured summary, expose failures/errors/exclusions, distinguish accepted baseline from live preview, contain no absolute paths or cross-bundle identifiers, and never load raw private artifacts in the portable view.
...
- Apply the slice-specific exit gates above. Final T46 maturity acceptance additionally requires: Golden Core risk/capability coverage toward the 50–64 target, all P0 hard gates passing, complete declared grader mutation detection, reference-program solvability without circular oracle generation, bounded and truthfully sourced costs, agent-first CLI/Skill acceptance, Repair Packet usability, presentation provenance parity and a fault-injection RCA baseline.
...
- Implementing all of Issue #93 as one change, or beginning a later slice before the preceding slice exit gate is met.
- Allowing a Coding Agent, ProjectFlow Agent or Judge to promote standards without Robert's approval.
- Claiming that local tooling cryptographically proves Robert's identity; approval is explicit repository governance and reviewable history.
...
- Allowing autonomous presets to reset an arbitrary caller-provided database, or treating a development-mode/path blacklist check as sufficient isolation.
- Editing the user worktree or production runtime to inject evaluation faults.
...
- Claiming a historical baseline delta, earliest divergence or deterministic symptom alone proves an implementation root cause.
...
- Migrating the repository to a root npm workspace solely for T46.
...
- Automatically deleting local evaluation evidence in V1.
...
- The Golden Core maturity target is approximately 50–64 high-quality canonical scenarios selected by capability/risk coverage. It is reached slice by slice only after oracle provenance, fixture solvability and grader mutation checks pass; generated paraphrase, boundary, adversarial and historical badcase variants expand coverage but do not create new truth.
...
- The presentation layer is evidence-backed, not a marketing-only page. The portable redacted bundle and local read viewer must expose coverage, failures, infrastructure errors, skipped cases, version provenance and reproducibility alongside positive results.
- The implementation audit found that the current fixture provisioner calls destructive demo seed/reset behavior, `APP_ENV=development` bypasses its admin token, and one sidecar process has one FastAPI base URL. The corrected design therefore requires positive evaluator ownership proof and a dedicated backend/sidecar pair per worker; it does not assume the existing fixture helper is already hermetic.
- The repository currently has separate backend, agent-bridge and frontend dependency boundaries plus a pinned Node wrapper, not a root JavaScript workspace. T46 preserves that structure unless a later independently justified architecture decision changes it.
