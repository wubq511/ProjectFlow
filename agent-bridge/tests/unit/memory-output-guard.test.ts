...
describe("memory-output-guard", () => {
  ...
  it("overrides an incorrect compliant review when the draft assigns a guarded member", async () => {
    const base = queuedStreamFn([
      message("前端页面开发由小林辅助完成。"),
      message('{"compliant":true,"violations":[]}'),
      message("前端页面开发暂无合适人选。"),
      message('{"compliant":true,"violations":[]}'),
    ]);
    const guarded = createMemoryGuardedStreamFn(base.streamFn, guardContext);
    ...
    expect(result.content).toEqual([{ type: "text", text: "前端页面开发暂无合适人选。" }]);
  });

  it("falls back when a repaired output still assigns a guarded member", async () => {
    const base = queuedStreamFn([
      message("后端由小林负责。"),
      message('{"compliant":false,"violations":["硬约束冲突"]}'),
      message("后端由小林在晚上辅助。"),
      message('{"compliant":true,"violations":[]}'),
    ]);
    const guarded = createMemoryGuardedStreamFn(base.streamFn, guardContext);
