...
- 2026-07-20 T46 Evaluation Lab Slice 4 closure: merged and closed Issue #99 at `a3df83d`, freezing a 52-scenario Golden Core across 8 capability domains and 8 scenario classes with P0 scope protection, robustness variants, candidate governance, deterministic fingerprints and a `$1` SUT cap. The light release gate aligned executable Skill IDs and ToolManifest effect types, kept prohibited commit effects fail-closed, passed Agent Bridge 2255/2255 plus typecheck/build, preserved the inherited `full` mock baseline at 16/16, and recorded an integrity-verified Golden Core baseline of 30/52 without weakening failures. Full cross-slice adversarial review and repair alignment remains deferred until all T46 tickets are implemented.
- 2026-07-20 T46 Evaluation Lab Slice 3 closure: merged and closed Issue #98 at `14e106e` with governed active/candidate standards, conflict blocking, criterion-scoped semantic evidence, deterministic anchor/bias calibration, fail-safe `needs_review`, separate SUT/evaluator/Coding Agent budgets and immutable calibration artifacts. Light release gate repaired the real mock CLI path, anchor-order transposition, promotion-eligibility consistency and exit-code classification; Agent Bridge 2170/2170 plus typecheck/build and the real mock `calibrate` command pass. Full cross-slice adversarial review remains deferred until all T46 tickets are implemented.
- 2026-07-20 T46 Evaluation Lab Slice 2 closure: merged and closed Issue #97 with evidence-graded diagnosis, evaluator-owned counterfactuals and fault profiles, a 12-sample anti-gaming RCA benchmark, evidence-bound clustering, immutable Repair Packets and governed Coding Agent prompts. Light release gate: Agent Bridge 1945/1945 plus typecheck/build; fault catalog and all RCA benchmark gates passed. Full cross-slice adversarial review is deferred until all T46 tickets are implemented.
...
- 2026-07-17 T46 Evaluation Lab Slice 0: added a repository-local Agent Skill and machine-readable `scripts/eval-lab` CLI, zero-token contract/config validation, evaluator-owned backend/sidecar process isolation, T43 public HTTP/SSE smoke reuse, bounded SUT cost/token/request/time enforcement, separate SUT/evaluator/Coding Agent ledgers, resumable immutable artifacts, provenance fingerprints, SHA-256 integrity verification, and adversarial coverage for nonce/instance/path/symlink/schema/SSE boundaries.
- 2026-07-16 Demo Seed V2 and Conversation UX: Seed data expanded from 10→11 tasks, 5→8 AgentEvents, 3→7 ProjectMemories (all V1 types, privacy boundary), added 5 AgentProposals (lifecycle), 3 AgentConversations (18 msgs), 1 AssignmentNegotiation, 12 TaskStatusUpdates, 3 AssignmentResponses, check-ins spread across 3 dates. Timeline aligned to real ProjectFlow development (05-28→07-17). Conversation history Sheet groups by month with collapsible sections. Chat messages default-collapse older history with expand/collapse toggle. De-paneling: direction_card suggestions/risks/unknowns/decision_points rewritten from defense to development language.
