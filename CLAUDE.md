...
T46 Evaluation Lab Issue #96（Slice 1 多轮 / Skill / Runtime / 可靠性）已于 2026-07-20 在 `glm/t46-96-conversation-runtime-reliability` 分支完成实现、独立对抗审查与修复（未 merge、未关闭 issue）：确定性多轮用户控制器、Simulator integrity、append-only Attempt ledger、Skill 8 维度、Runtime 11 故障类、demo/smoke/smoke-v2/full presets、真实 candidate/baseline 隔离执行、Reliability statistics、Operational metrics 分账和 6 条件 exit gate 已接入稳定 CLI 与真实 runner。mock `demo` 5/5、`full` 16/16，`exit-gate` 6/6；单次 full 的 `reliability` 必须报告 evidence 不足，不能把不同场景当 repetitions。paired runner 使用独立 detached worktree/backend/sidecar/database/temp/artifact，无法确认 resolved model 时 fail-closed 为 model drift possible。SUT `full` 成本为 `$0.00`，Coding Agent 成本保持 `unknown/null` 并分账。付费模型不属于 #96 关闭条件；在冻结价格表与调用前最坏成本前继续 fail-closed。#96 合并前不得宣称 Slice 1 已关闭或开始 Slice 2。后续先读 `docs/T46/ProjectFlow_Agent_Evaluation_Lab_Spec.md` 与 Slice 0/1 handoff。
...
> 当前阶段：Issue #96 已在 `glm/t46-96-conversation-runtime-reliability` 分支完成实现与独立修复，尚未 merge、未关闭 issue；Slice 1 关闭还需当前分支最终回归通过并 merge。付费模型 canary 是可选校准，不是关闭前提；在价格和调用前上限冻结前继续 fail-closed。不得提前进入 Slice 2、Dashboard、语义 Judge 或自动 RCA/Repair Packet。Proposal-Confirm、会话隐私和业务事实边界不变。
