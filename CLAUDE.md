...
T46 Evaluation Lab Issue #95（Slice 1 hard-domain foundation）已于 2026-07-19 在 `glm/t46-95-hard-oracles` 分支实现：evaluation-only normalized evidence seam 强绑定 workspace/project/viewer/conversation/run；Scenario Contract V2 使用 event/tool nodes + causal edges 表达 strict/unordered/subset/superset DAG；15 个确定性 hard grader 分离 Outcome / Authority & Safety / Trajectory / Privacy，缺轨迹、重复终态、未知 side effect 均 fail-closed。`smoke-v2` 通过真实隔离 public HTTP/SSE 行为入口和公开 Proposal API 覆盖 answer-only、plan-confirm、plan-reject；Reference Programs 使用完整 oracle 与 before/pre-human/final 快照。隐藏 sentinel 只以 length+SHA-256 probe 检查 request/context/trace，portable manifest 仅保存 digest。Issue #96 仍需完成多轮、Skill/Runtime、可靠性和 candidate/baseline 报告，#96 完成前不得宣称整个 Slice 1 关闭或开始 Slice 2。后续先读 `docs/T46/ProjectFlow_Agent_Evaluation_Lab_Spec.md` 与 Slice 0/1 handoff。
...
T44/T45 之后的 2026-07-14 UX/Runtime 打磨完成设置入口迁移、AgentSidebar 精简、设置页重构、斜杠命令高亮和 Composer steering。2026-07-16 演示种子数据 V2 完成完整生命周期轨迹。2026-07-17 T46 Slice 0 合并到 `main`；2026-07-19 Issue #95 在 `glm/t46-95-hard-oracles` 分支完成 Slice 1 hard-domain foundation，并通过真实隔离 `smoke-v2` 的 answer/confirm/reject 与三条 Reference path。详见 `docs/demo-script.md`、Slice 0/1 handoff。

> 当前阶段：Issue #95 已完成但未合并；下一 ticket 是 #96，完成后才关闭 Slice 1。不得提前进入 Slice 2、Dashboard、语义 Judge 或自动 RCA/Repair Packet。Flash 保持默认，Pro 只作显式质量升级；Proposal-Confirm、会话隐私和业务事实边界不变。
