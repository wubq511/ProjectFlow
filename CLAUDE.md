...
T46 Evaluation Lab Issue #95（Slice 1 hard-domain foundation）已于 2026-07-19 合并到 `main`：evaluation-only normalized evidence seam 强绑定 workspace/project/viewer/conversation/run；Scenario Contract V2 使用 event/tool nodes + causal edges 表达 strict/unordered/subset/superset DAG；15 个确定性 hard grader 分离 Outcome / Authority & Safety / Trajectory / Privacy，缺轨迹、重复终态、未知 side effect 均 fail-closed。`smoke-v2` 通过真实隔离 public HTTP/SSE 行为入口和公开 Proposal API 覆盖 answer-only、plan-confirm、plan-reject；Reference Programs 使用完整 oracle 与 before/pre-human/final 快照。隐藏 sentinel 只以 length+SHA-256 probe 检查 request/context/trace，portable manifest 仅保存 digest。

T46 Evaluation Lab Issue #96（Slice 1 多轮 / Skill / Runtime / 可靠性）已于 2026-07-20 合并到 `main`（`50c0f77`）并关闭：确定性多轮用户控制器、Simulator integrity、append-only Attempt ledger、Skill 8 维度、Runtime 11 故障类、demo/smoke/smoke-v2/full presets、真实 candidate/baseline 隔离执行、Reliability statistics、Operational metrics 分账和 6 条件 exit gate 已接入稳定 CLI 与真实 runner。mock `demo` 5/5、`full` 16/16，`exit-gate` 6/6；单次 full 的 `reliability` 必须报告 evidence 不足，不能把不同场景当 repetitions。paired runner 使用独立 detached worktree/backend/sidecar/database/temp/artifact，无法确认 resolved model 时 fail-closed 为 model drift possible。SUT `full` 成本为 `$0.00`，Coding Agent 成本保持 `unknown/null` 并分账。付费模型不属于 #96 关闭条件；在冻结价格表与调用前最坏成本前继续 fail-closed。其后续 Issue #97 也已完成并合并。

T46 Evaluation Lab Issue #97（Slice 2 Diagnosis & Repair）已于 2026-07-20 合并到 `main`（`3e09596`）并关闭：将可信评测失败转化为 evidence-graded diagnosis、evaluator-owned 单变量 counterfactual 与 fault injection 证据、known-fault RCA benchmark、evidence-bound issue clustering、immutable Repair Packet 和 copy-ready 修复 prompt。5 个冻结诊断状态、8 类 fault profiles、5 个 anti-gaming benchmark gates、fix/investigation 门禁、内容 scrub、stale detection 和 candidate regression governance 均已进入稳定 CLI 与 artifact store。

T46 Evaluation Lab Issue #98（Slice 3 Governed Calibration & Semantic Standards）已于 2026-07-20 合并到 `main`（`14e106e`）并关闭：governed calibration pipeline 与 criterion-scoped semantic rubric 已进入稳定 CLI。Active/candidate registries 严格分离；6 类 frozen standard conflict pattern 阻止未解决或 deferred 冲突 promotion；5 类 semantic verdict、6 类 bias metric 与 9 类 fail-safe 保持 deterministic hard gates 优先；`calibrate` 的 `$3` 只约束 SUT，evaluator 与 Coding Agent 独立分账；immutable calibration artifact 进入 SHA-256 result graph；`applyPromotionApproval` 是唯一 active mutation path，要求显式 Robert instruction、reviewable Git diff、matching fingerprints 和全冲突 resolved。轻量发布门禁额外修复真实 mock CLI 未传 Judge evidence、anchor-order 分组转置、失败 candidate 仍显示 eligible、以及非预算失败误报 exit `4`；Agent Bridge 2170/2170、typecheck/build、preset validation、conflict catalog 与真实 deterministic `calibrate` 通过。付费 Judge 在冻结价格表和调用前最坏成本前继续 fail-closed。本 ticket 未做跨 Slice 全面审查；统一审查与修复按用户要求留到全部 T46 tickets 完成后。

T46 Evaluation Lab Issue #99（Slice 4 Golden Core Expansion & Freeze）已于 2026-07-20 在分支 `glm/t46-99-golden-core` 完成实现：将现有 Evaluation Lab 从 16 个场景扩展为冻结的 ProjectFlow Golden Core，52 个 canonical scenarios 覆盖 8 capability domains × 8 scenario classes；单一、版本化、可审查的 Golden Core registry（TS 为事实源，JSON snapshot 为冻结审计产物）；`GOLDEN_CORE_DEFAULT_FROZEN_AT = "2026-07-20T00:00:00.000Z"` 确定性 fingerprint；`computeRegistryFingerprint` 包含 `candidates`/`rejected`/`freezeNotes` 字段；每个 stateful scenario 的 9 项 trusted entry conditions（goalProvenance、fixtureSeed+fixtureFingerprint、goldenConstraintsSummary、referenceProgramId、declaredGraderMutations、mutationDetectionEvidence、scope、stateEffectSummary、milestoneDagSummary）；P0 不可移除集合（8 类：safety-authority、privacy-visibility、proposal-confirm、idempotency、forbidden-side-effects、terminal-consistency、read-only-purity、hidden-field-leakage）；P0 scope filter protection 阻止 `--scenario`/`--exclude` 静默移除 P0 场景；6 类 robustness variants（semantic-paraphrase、distraction-injection、description-weakening、irrelevant-context、order-variation、bounded-adversarial-wording）不膨胀 canonical count；generated regression candidate governance（candidate registry only，8 项 promotion 验证：representativeness、redaction、hiddenGoalIntegrity、nonDuplicationWithCanonical、fixtureSolvability、graderMutationDeclaration、reviewableDiff、explicitApprovalRecord）；`golden-core` preset 与 SUT `$1` cap（同 `full`）+ 独立 evaluator ceiling + Coding Agent external/unknown；`golden-core` CLI 子命令 `freeze`/`verify`/`coverage`/`list`/`candidates`；validation.ts 识别 `golden-core` preset 应用 `$1` cap；`unknownSideEffects="fail_closed"` 时强制要求非空 `allowedSideEffectTypes`（防止 grader 静默 skip fail-open）。Hard deterministic gates 优先；复用 T43 public HTTP/SSE seam；grader 不调用 SUT 业务实现；fixture/goal/oracle/Reference Program/grader mutation 逻辑独立。本 ticket 不做跨 Slice 全面审查；不执行 Issue #100 的 Dashboard/viewer 工作；不执行 active-standard promotion；不调用付费模型。后续先读 `docs/T46/ProjectFlow_Agent_Evaluation_Lab_Slice4_Handoff.md` 和 canonical spec。
...
**Next.js Frontend + FastAPI Backend + SQLite + TypeScript Agent Bridge/Pi Runtime + Proposal-Confirm State Machine**

- 前端 Next.js (React + TypeScript + Tailwind CSS + shadcn/ui + Framer Motion)
- 后端 FastAPI (Python + SQLModel + Pydantic)
- 数据库 SQLite（本地演示优先，零配置）
- Sidecar/Pi runtime 是主要 Agent 路径；legacy `CoordinatorAgent` 仅保留为迁移/回退资产
...
- sidecar 管 runtime loop、provider routing（多模型多供应商注册表 + 动态 import）、tool registry、policy gate、event bridge、trace envelope。
...
│   │   ├── config/          # model-config-store.ts, dotenv-writer.ts, file-watcher.ts
...
│   │   ├── types/           # run-state.ts, tool-manifest.ts, tool-result.ts, wire.ts, runtime-event.ts, model-config.ts
│   │   └── utils/           # 工具函数
│   ├── skills/              # 6 SKILL.md files (project-intake/planning/task-breakdown/assignment/risk-replan/status)
│   ├── model-configs.json   # 多模型配置注册表（默认 4 条：DeepSeek V4 Flash/Pro, MiMo V2.5, MiMo V2.5 CN）
│   └── tests/unit/          # 60 test files, 1198 tests
├── frontend/
│   ├── src/
│   │   ├── app/             # Next.js 页面路由（不写业务逻辑）
│   │   ├── components/      # 按业务领域拆分：ui/ onboarding/ workspace/ project/ member/ agent/ stage/ task/ assignment/ checkin/ risk/ settings/；项目页采用三栏布局(project-layout + project-sidebar + project-content + agent-sidebar)
│   │   ├── lib/
...
│   │   ├── memory/     # T42 ProjectMemory: extractor, display_resolver, retriever (FTS5/jieba/2-phase), context_builder, vector_retriever (lazy), retrieval_eval, query_normalizer, ab_eval
...
- **AgentConversation / AgentMessage**: 项目内会话与消息；新会话 creator-owned/private，旧会话 team-visible，消息游标分页；会话不是 ProjectMemory 或 Primary Project State
...
MODEL_CONFIGS_PATH=           # sidecar 模型配置文件路径，默认 ../../model-configs.json
DOTENV_PATH=                  # sidecar .env 文件路径（API key 写入目标），默认 ../../.env
...
Phase 0-39 全部已完成（2026-05-28 ~ 2026-06-07）。Phase 40（2026-06-07）为 Agent sidebar UI 打磨阶段。Phase 41（2026-06-08）为全面安全审查与性能优化。T41 Agent Runtime 架构方案已在 2026-07-04 确认并提交，T41 runtime/tool slices 已实现到 S16。T42 ProjectMemory V1 issues #71-#80 已在 2026-07-07 合并；remediation Batches A-D（R1-R6, R8）及选择性 R8 Pilot 已在 2026-07-11 完成并通过 7/7 gates；R7（optional vector V1.1）为独立项目。多模型多供应商配置与切换已完成（2026-07-08）。T43 Agent Harness V2 P0 已在 2026-07-12 完成仓库级实现与验证，DeepSeek Flash/Pro production canary 已于 2026-07-13 通过。T44 Agent 效率/模型完整性和 T45 私人多会话历史于 2026-07-13 完成确定性实现、全量回归和 T44 重复生产 canary。

T44/T45 之后的 2026-07-14 UX/Runtime 打磨完成设置入口迁移、AgentSidebar 精简、设置页重构、斜杠命令高亮和 Composer steering。2026-07-16 演示种子数据 V2 完成完整生命周期轨迹。2026-07-17 T46 Slice 0 合并到 `main`；2026-07-19 Issue #95 合并；2026-07-20 Issues #96-#98 合并并关闭，Slice 1、Slice 2 与 Slice 3 governed calibration 完成；2026-07-20 Issue #99 在分支 `glm/t46-99-golden-core` 完成 Golden Core 扩展与冻结实现（52 个 canonical scenarios、8 capability × 8 scenario class coverage、P0 不可移除集合、6 类 robustness variants、candidate governance、`golden-core` CLI）。详见 `docs/demo-script.md`、T46 Slice handoff 与 canonical spec。

> 当前阶段：Issues #94-#98 已合并关闭；Issue #99 在分支 `glm/t46-99-golden-core` 完成实现并本地 commit（未 push、未合并、未关闭）。#99 的轻量门禁为 Agent Bridge 2239/2249（10 个预存在的 Node 版本不匹配失败与 #99 无关）、typecheck/build、`validate --preset golden-core` 仅剩预存在的 Node 版本失败、t46-6 79/79 测试通过。全面对抗审查与跨切片修复对齐在全部 T46 tickets 完成后统一进行。
