...
T46 Evaluation Lab Issue #96（Slice 1 多轮 / Skill / Runtime / 可靠性）已于 2026-07-20 在 `glm/t46-96-conversation-runtime-reliability` 分支完成确定性实现与对抗性自审（未 merge、未关闭 issue）：(1) 确定性多轮用户控制器 `UserController`（hidden facts/sentinels 仅存 evaluator 内存，phrasing 函数不能改 facts，refusal/goal_drift/hidden_fact_leak 三类 simulator_error）；(2) Simulator integrity（`simulator-error.ts` 把 simulator_error 从分母排除，`SIMULATOR_RETRY_BUDGET=2` 冻结常量）；(3) Attempt ledger（append-only，仅成功 retry 写 `recoveredBy`，拒绝覆盖已有指针，防 retry 抹除先前失败证据）；(4) Skill evaluation 8 维度（positive_trigger / negative_trigger_not_fired / prerequisites_satisfied / allowed_tools / required_steps / forbidden_actions / fallback_behavior / effect_ceiling，effect ceiling 复用 Skills V2 单一权威）；(5) Runtime reliability 11 故障类（duplicate_terminal / contradictory_terminal / cancellation_completed / resume_side_effect_dup / missing_terminal / unknown_side_effect / tool_error_unhandled / tool_result_lost / state_repair_during_read / invalid_state_transition / tool_manifest_duplicate）；(6) Presets（demo/smoke/smoke-v2/full + T46_3_P0_SCENARIO_IDS，budget cap: smoke $0.10 / full $1 / calibrate $3）；(7) Candidate/baseline paired comparison（worktree/backend port/sidecar port/nonce/instance/database/temp root/artifact staging 8 资源隔离检测，`resolvedModel.confirmedBy` 拒绝 requested/assumed/default/unconfirmed/unknown/空字符串，防 requested 冒充 resolved；candidateWins 需充分证据 + 无 model drift + candidate 严格高于 baseline）；(8) Reliability statistics 6 指标（observed_trial_pass_rate / empirical_all_k_reliability / pass_at_k / modeled_pass_k / all_invariant_pass / confidence_interval，statistical significance 仅 full preset 且 n≥30 才允许声明）；(9) Operational metrics 分账（sutCost 计入 SUT cap，evaluatorModelCost 与 codingAgentCost 不计入）；(10) Slice 1 exit gate 6 条件全 fail-closed（P0 mutation 检测 / reference program 无 hard false failure / hidden field 无泄漏 / required scenario 不被 skipped/excluded/fail 美化 / evidence integrity checksums+graph / no semantic judge）。CLI 新增 `exit-gate <run-id> [--json]` 与 `reliability <run-id> [--confidence-level 0.95] [--json]`。15 类对抗性自审发现并修复 3 个 bug（attempt-ledger failed retry 误设 recoveredBy、CLI exit-gate requiredScenarios 仅检查观察未检查 hard grade、paired-comparison confirmedBy 允许 requested）；40 个 mutation test 覆盖 8 个新模块所有声明 grader/checker。验收基线：890 backend passed / 4 skipped + ruff；1628 agent-bridge passed（75 files）+ typecheck + build；333 frontend passed / 6 skipped（26 files）+ lint + build；demo/smoke/smoke-v2/full 4 preset validate 全通过；smoke 1/1、smoke-v2 3/3、demo 3/6（T46-3 新场景需 runner 集成，属预期）真实本地运行通过。Slice 1 整体关闭仍需 #96 merge + 真实付费模型 canary；#96 完成前不得宣称整个 Slice 1 关闭或开始 Slice 2。后续先读 `docs/T46/ProjectFlow_Agent_Evaluation_Lab_Spec.md` 与 Slice 0/1 handoff。
...
T44/T45 之后的 2026-07-14 UX/Runtime 打磨完成设置入口迁移、AgentSidebar 精简、设置页重构、斜杠命令高亮和 Composer steering。2026-07-16 演示种子数据 V2 完成完整生命周期轨迹。2026-07-17 T46 Slice 0 合并到 `main`；2026-07-19 Issue #95 的 Slice 1 hard-domain foundation 合并到 `main`，并通过真实隔离 `smoke-v2` 的 answer/confirm/reject 与三条 Reference path。2026-07-20 Issue #96 在 `glm/t46-96-conversation-runtime-reliability` 分支完成多轮用户控制器、Simulator integrity、Skill 8 维度、Runtime 11 故障类、Attempt ledger、demo/smoke/smoke-v2/full presets、Candidate/baseline paired comparison、Reliability statistics、Operational metrics、Slice 1 exit gate 的确定性实现与对抗性自审。详见 `docs/demo-script.md`、Slice 0/1 handoff。

> 当前阶段：Issue #96 已在 `glm/t46-96-conversation-runtime-reliability` 分支完成实现、对抗性自审与全量验证，尚未 merge、未关闭 issue；Slice 1 整体关闭仍需 #96 merge + 真实付费模型 canary。不得提前进入 Slice 2、Dashboard、语义 Judge 或自动 RCA/Repair Packet。Flash 保持默认，Pro 只作显式质量升级；Proposal-Confirm、会话隐私和业务事实边界不变。
