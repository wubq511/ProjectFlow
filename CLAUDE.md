...
T46 Evaluation Lab Issue #96（Slice 1 多轮 / Skill / Runtime / 可靠性）已于 2026-07-20 合并到 `main`（`50c0f77`）并关闭：确定性多轮用户控制器、Simulator integrity、append-only Attempt ledger、Skill 8 维度、Runtime 11 故障类、demo/smoke/smoke-v2/full presets、真实 candidate/baseline 隔离执行、Reliability statistics、Operational metrics 分账和 6 条件 exit gate 已接入稳定 CLI 与真实 runner。mock `demo` 5/5、`full` 16/16，`exit-gate` 6/6；单次 full 的 `reliability` 必须报告 evidence 不足，不能把不同场景当 repetitions。paired runner 使用独立 detached worktree/backend/sidecar/database/temp/artifact，无法确认 resolved model 时 fail-closed 为 model drift possible。SUT `full` 成本为 `$0.00`，Coding Agent 成本保持 `unknown/null` 并分账。付费模型不属于 #96 关闭条件；在冻结价格表与调用前最坏成本前继续 fail-closed。下一切片是 Issue #97 的 evidence-graded RCA 与 Repair Packets。

T46 Evaluation Lab Issue #97（Slice 2 Diagnosis & Repair）已于 2026-07-20 在分支 `glm/t46-97-rca-repair-packets` 上实现（未合并）：将可信评测失败转化为 evidence-graded diagnosis、evaluator-owned 单变量 counterfactual 与 fault injection 证据、可度量的 known-fault RCA benchmark、有证据约束的 issue clustering、Coding Agent 可直接消费的 immutable Repair Packet 和 copy-ready 修复 prompt。5 个冻结诊断状态 `observed_failure | localized_hypothesis | intervention_supported | fault_injection_confirmed | unresolved`，`intervention_supported` 不能升级到 `fault_injection_confirmed`，必须从 `localized_hypothesis` 直接升级。8 类 evaluator-owned fault profile categories（routing/context/skill/tool_schema_or_result/policy_or_effect_boundary/privacy_or_visibility/proposal_evidence/terminal_events）。RCA benchmark 3 个 sample classes（correct_attribution/confusable_neighbour/unresolved_or_insufficient）与 5 个 gates（top1Accuracy ≥ 0.5、falseAttributionRate ≤ 0.3、confidenceCalibration ≥ 0.7、evidenceCompleteness ≥ 0.7、top3Recall − top1Accuracy ≤ 0.4）；anti-gating 规则：correct_attribution 样本的 top3Correct 只在 top1Correct 也为 true 时才计数。`REPAIR_PACKET_SCHEMA_VERSION = 1`；`fix` packet 需要 direct component evidence 或 intervention_supported 或 fault_injection_confirmed，且必须同时具备 falsifiable acceptance test、protected boundaries、verification commands、有效 code fingerprint，否则降级为 `investigation`。Packet 内容 scrub：secrets（api_key/token/password/bearer）、绝对临时路径、raw hidden fact 标记（`__hidden__`/`__oracle__`/`__expected_cause__`）、model hidden reasoning（`<think>`/`<reasoning>`）。Stale detection：commit 或 worktree SHA-256 不一致时 fail-closed 为 `stale`。Candidate regression 标记为 `candidate`/`unapproved`，不能 auto-promote 为 `approved`。Coding Agent prompt 禁止修改 `.env`、API key、internal service token、frozen standards、graders、thresholds、P0 cases、隐私/权威边界；禁止 auto push/merge/Issue close；stale packet 拒绝执行。10 个新测试文件（317 tests，含 46 mutation tests）覆盖 contract、fault profiles、earliest divergence、counterfactual、RCA benchmark anti-gaming、issue clustering、repair packet invariants、repair prompt invariants、end-to-end pipeline、pipeline-level invariants与对称性补全。真实 CLI 路径已验证：`diagnose` → `diagnosis_skipped`（全 pass run）、`rca-benchmark` → 12 samples 通过 5 gates、`exit-gate` → 6/6、`fault-catalog` → `complete: true`、`repair-packet` → `repair_packets_empty`（无 diagnosis）。付费模型不属于 #97 关闭条件；Proposal-Confirm、FastAPI 事实源、sidecar 不读写业务 DB、public HTTP/SSE seam、evaluator-owned isolation、immutable checkpoint/SHA-256 和 Slice 0/1 兼容边界不变。后续 Slice 2 工作先读 `docs/T46/ProjectFlow_Agent_Evaluation_Lab_Spec.md` 的 Slice 2 handoff 部分。
...
T44/T45 之后的 2026-07-14 UX/Runtime 打磨完成设置入口迁移、AgentSidebar 精简、设置页重构、斜杠命令高亮和 Composer steering。2026-07-16 演示种子数据 V2 完成完整生命周期轨迹。2026-07-17 T46 Slice 0 合并到 `main`；2026-07-19 Issue #95 的 Slice 1 hard-domain foundation 合并到 `main`；2026-07-20 Issue #96 的多轮、Skill、Runtime、可靠性、paired comparison 与 exit gate 合并到 `main` 并关闭；2026-07-20 Issue #97 的 Slice 2 Diagnosis & Repair 在分支 `glm/t46-97-rca-repair-packets` 上实现（未合并），覆盖 evidence-graded diagnosis、evaluator-owned counterfactual 与 fault injection、known-fault RCA benchmark（5 anti-gaming gates）、issue clustering、immutable Repair Packet（schema version 1，fix/investigation gate，scrub，stale detection，candidate regression governance）和 Coding Agent 修复 prompt。详见 `docs/demo-script.md`、Slice 0/1 handoff 与 `docs/T46/ProjectFlow_Agent_Evaluation_Lab_Spec.md` 的 Slice 2 handoff 部分。

> 当前阶段：Issue #96 已合并并关闭，Slice 1 完成。Issue #97（Slice 2 Diagnosis & Repair）已在分支 `glm/t46-97-rca-repair-packets` 上实现（2026-07-20），等待本地验证与 commit，未 push、未 merge、未关闭 Issue。后续 Slice 2 工作必须先读 `docs/T46/ProjectFlow_Agent_Evaluation_Lab_Spec.md` 的 Slice 2 handoff 与 Pre-Implementation Adversarial Review，不得跳过 Slice 1 trust boundary 直接扩展 Dashboard 或语义 Judge。付费模型在价格和调用前上限冻结前继续 fail-closed；Proposal-Confirm、会话隐私、业务事实边界和 frozen standards 不变。
