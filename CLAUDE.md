...
MVP 状态：初步闭环已跑通（Phase 0-36），演示 Demo 可用，可用性打磨阶段（Phase 37-39）完成。
...
阶段自动推进：任务标记 done 时 `try_advance_stage()` 检测该阶段所有任务是否完成，全部完成则自动标记阶段 completed 并激活下一个 pending 阶段。无更多阶段时项目标记为 completed。
...
Phase 0-39 全部已完成（2026-05-28 ~ 2026-06-07）。Phase 40（2026-06-07）为 Agent sidebar UI 打磨阶段。

> 当前阶段：Agent sidebar 动画、artifact 交互、Planner 重试逻辑、PRD 对照修复（方向卡字段、导出中文、卡片样式统一、内联确认、复盘 Agent 总结）已完成。
