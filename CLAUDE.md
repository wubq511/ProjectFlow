...
T46 Evaluation Lab Issue #97（Slice 2 Diagnosis & Repair）已于 2026-07-20 合并到 `main`（`3e09596`）并关闭：将可信评测失败转化为 evidence-graded diagnosis、evaluator-owned 单变量 counterfactual 与 fault injection 证据、known-fault RCA benchmark、evidence-bound issue clustering、immutable Repair Packet 和 copy-ready 修复 prompt。5 个冻结诊断状态、8 类 fault profiles、5 个 anti-gaming benchmark gates、fix/investigation 门禁、内容 scrub、stale detection 和 candidate regression governance 均已进入稳定 CLI 与 artifact store。轻量发布门禁通过 Agent Bridge 1945/1945、typecheck/build、fault catalog completeness 和 12-sample RCA benchmark；全面对抗审查按用户要求留到全部 T46 tickets 完成后统一进行。下一 ticket 是 Issue #98 的 governed calibration 与 semantic standards。

T46 Evaluation Lab Issue #98（Slice 3 Governed Calibration & Semantic Standards）已于 2026-07-20 在本地分支 `glm/t46-98-governed-calibration` 完成确定性实现（本地 commit 已创建；未 push、未 merge、未关闭）：governed calibration pipeline 与 criterion-scoped semantic rubric。Active/candidate standards registries 严格分离（`agent-bridge/standards/active/registry.json` 只读，`agent-bridge/standards/candidate/` 只由 calibration 写）；`loadActiveRegistry` ENOENT 时返回稳定 bootstrap registry（固定 timestamp `1970-01-01T00:00:00.000Z`）；6 类 frozen standard conflict pattern（`canonical-vs-adr` / `canonical-vs-schema` / `canonical-vs-public-behavior` / `schema-vs-code` / `frozen-scenario-vs-code` / `frozen-standard-vs-candidate`）检测并阻止 promotion；`hasUnresolvedConflict` 把 `unresolved` 与 `deferred` 都视为阻塞，只有 `resolved` 才能解锁；criterion-scoped semantic rubric 5 类 verdict（`pass | fail | needs_review | infra_error | insufficient_evidence`）；blinding + seed-randomized A/B order + reverse repetition + 6 类 bias metric（position / verbosity / same-family / disagreement / repeated-run / anchor-ordering）；good/boundary/bad anchors 与 Judge 稳定性测试（`computeAnchorStability` 按 anchor kind 推导 expected verdict）；9 类 fail-safe 条件按优先级降级到 `needs_review`（`no_independent_judge` → `judge_identity_unconfirmed` → `only_same_family_uncalibrated` → `judges_conflict` → `anchor_ordering_unstable` → `bias_metrics_exceeded` → `judge_telemetry_incomplete` → `judge_schema_unrepairable` → `calibration_evidence_insufficient`）；`calibrate` preset 与 SUT `$3` 上限 + 独立 evaluator ceiling + 三类 cost provenance（`provider_reported` / `versioned_price_estimate` / `unknown`）；immutable calibration artifact 进入 SHA-256 result graph；`applyPromotionApproval` 是唯一 active registry mutation path，需要显式 Robert instruction + reviewable Git diff + matching fingerprints + all conflicts resolved。ProjectFlow deterministic hard gates 永远优先；`combineHardGateWithSemantic(false, any)` 永远产生 `fail`；`semanticHardGateEligible` 默认 `false`，必须由独立校准证据单独提升。8 个新测试文件 221 个测试全部通过；实现者级别自审修复 8 个 bug（JSDoc `*/` glob trap ×2、变量遮蔽、`generateConflictId` 字段访问、`buildEmptyActiveRegistry` 不稳定 timestamp、`isEligibleForPromotion` 缺 `entry.id` 检查、`computeAnchorStability` 默认 verdict、`hasUnresolvedConflict` deferred 语义、`applyPromotionApproval` 测试 flakiness）。本 ticket 验收使用 mock/deterministic Judge，不运行真实付费模型。后续先读 `docs/T46/ProjectFlow_Agent_Evaluation_Lab_Slice3_Handoff.md`；全面对抗审查按用户要求留到全部 T46 tickets 完成后统一进行。
...
T44/T45 之后的 2026-07-14 UX/Runtime 打磨完成设置入口迁移、AgentSidebar 精简、设置页重构、斜杠命令高亮和 Composer steering。2026-07-16 演示种子数据 V2 完成完整生命周期轨迹。2026-07-17 T46 Slice 0 合并到 `main`；2026-07-19 Issue #95 合并；2026-07-20 Issues #96/#97 合并并关闭，Slice 1 与 Slice 2 完成。详见 `docs/demo-script.md`、Slice 0/1 handoff 与 T46 canonical spec。

> 当前阶段：Issue #98 已在本地分支 `glm/t46-98-governed-calibration` 完成确定性实现与本地 commit（未 push/merge/close）。Slice 3 governed calibration 与 semantic standards 完成：active/candidate registries 严格分离、6 类 frozen conflict pattern、criterion-scoped semantic rubric、6 类 bias metric、9 类 fail-safe、calibrate preset（SUT `$3`）、immutable calibration artifact、`applyPromotionApproval` 唯一 active mutation path。8 个新测试文件 221 测试通过；mock Judge 完整 pipeline 跑通。全面对抗审查与跨切片修复对齐在全部 T46 tickets 完成后统一进行。
