...
T46 Evaluation Lab Issue #99（Slice 4 Golden Core Expansion & Freeze）已于 2026-07-20 合并到 `main`（`a3df83d`）并关闭：将现有 Evaluation Lab 从 16 个场景扩展为冻结的 ProjectFlow Golden Core，52 个 canonical scenarios 覆盖 8 capability domains × 8 scenario classes；单一、版本化、可审查的 Golden Core registry（TS 为事实源，JSON snapshot 为冻结审计产物）；每个 stateful scenario 声明 9 项 trusted entry conditions；P0 不可移除集合覆盖 safety/authority、privacy、Proposal-Confirm、idempotency、forbidden effects、terminal consistency、read-only purity 与 hidden leakage；6 类 robustness variants 不膨胀 canonical count；generated regression candidates 需 8 项审查且不能自动 promotion；`golden-core` preset 复用 `full` 的 `$1` SUT cap，并与 evaluator/Coding Agent 成本分账。轻量门禁修复了可执行契约中的无效 Skill ID、ToolManifest effect type 漂移和 prohibited commit effect 分类，新增回归守卫；Agent Bridge 2255/2255、typecheck/build、`full` mock 16/16 与 mock `calibrate` 通过。真实 Golden Core public-seam mock 基线为 30/52，制品完整性通过；剩余失败保留为最终跨 Slice 审查与修复对齐的输入，不通过弱化 grader 消除。Issue #100 是下一项；付费模型和 active-standard promotion 仍不在默认路径。
...
T44/T45 之后的 2026-07-14 UX/Runtime 打磨完成设置入口迁移、AgentSidebar 精简、设置页重构、斜杠命令高亮和 Composer steering。2026-07-16 演示种子数据 V2 完成完整生命周期轨迹。2026-07-17 T46 Slice 0 合并到 `main`；2026-07-19 Issue #95 合并；2026-07-20 Issues #96-#99 合并并关闭，Slice 1、Slice 2、Slice 3 governed calibration 与 Slice 4 Golden Core 完成。详见 `docs/demo-script.md`、T46 Slice handoff 与 canonical spec。

> 当前阶段：Issues #94-#99 已合并关闭；下一项为 Issue #100 evidence-backed showcase 与 T46 closeout。#99 轻量门禁为 Agent Bridge 2255/2255、typecheck/build、Golden Core targeted 137/137、`full` mock 16/16、mock `calibrate` 通过；Golden Core mock 基线 30/52 且完整性通过。全面对抗审查与跨切片修复对齐在全部 T46 tickets 完成后统一进行。
