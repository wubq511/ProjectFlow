...
T46 Evaluation Lab Slice 1（Issue #95）已于 2026-07-19 在 `glm/t46-95-hard-oracles` 分支实现：新增 normalized read-only viewer-scoped evidence snapshot endpoint（`GET /internal/evaluation/evidence`，认证复用 Slice 0 evaluator identity + internal service token）、Scenario Contract V2（`HardGraderContract`、`EvidenceSnapshot`、`ReferenceProgram`、`HardGrade`）、15 个确定性 hard grader 分布在 Outcome / Authority & Safety / Trajectory / Privacy 四维度（hard-gate failure 不可被其他维度抵消）、oracle/reference independence 校验、mutation primitive + suite 验证每个声明 grader 都能检测其目标失败、`smoke-v2` preset（opt-in，未声明 `hardGrader` 的场景保留 Slice 0 行为）。Hard grader 是纯函数，不 import runtime/router/verifier/业务 service，不调用 LLM；evidence endpoint 只读，不创建数据，不替代 Proposal-Confirm。实施后做了一轮对抗审查并修复 C-01/H-01/H-02/H-03/M-01/M-03/M-04/L-01；M-02（`SideEffectFacts` 缺 `event_seq`）延迟到后续 hard-domain suite。当前验收基线：885 backend passed / 4 skipped（含 19 个 evidence 测试）、1323 agent-bridge passed（65 files，3 个为 Slice 0 已存在的 Node 26 vs 锁定 24.15.0 环境失败）、333 frontend passed / 6 skipped（26 files）；backend ruff、agent-bridge typecheck/build 通过。后续 Slice 1 工作或 hard-domain suite 扩展先读 `docs/T46/ProjectFlow_Agent_Evaluation_Lab_Spec.md`、Slice 0/1 handoff；不得提前进入 Slice 2 诊断/RCA/Repair Packet、Slice 3 语义 Judge、Slice 4 Dashboard。
...
T44/T45 之后的 2026-07-14 UX/Runtime 打磨完成设置入口迁移、AgentSidebar 精简、设置页重构、斜杠命令高亮和 Composer steering。2026-07-16 演示种子数据 V2 完成 11 任务、8 Agent 事件、7 条 ProjectMemory、5 AgentProposal、3 团队会话 18 消息及完整生命周期轨迹。2026-07-17 T46 Evaluation Lab Slice 0 完成可信最小闭环并合并到 `main`；2026-07-19 T46 Slice 1（Issue #95）在 `glm/t46-95-hard-oracles` 分支实现 normalized evidence snapshot、Scenario Contract V2、15 个确定性 hard grader、oracle/reference 独立性、mutation validation 和 `smoke-v2` preset；当前基线为 885 backend passed / 4 skipped、1323 agent-bridge passed（65 files，3 个为 Slice 0 已存在的 Node 26 vs 24.15.0 环境失败）、333 frontend passed / 6 skipped（26 files）。详见 `docs/demo-script.md`、`docs/T46/ProjectFlow_Agent_Evaluation_Lab_Slice0_Handoff.md`、`docs/T46/ProjectFlow_Agent_Evaluation_Lab_Slice1_Handoff.md`。

> 当前阶段：T46 Evaluation Lab Slice 1 已在 `glm/t46-95-hard-oracles` 分支实现完成（未合并）。下一评测切片是 Slice 2（Diagnosis and Repair Handoff）；不得提前进入 Dashboard、语义 Judge、自动 RCA 或 Repair Packet。Flash 保持默认，Pro 只作显式质量升级；Proposal-Confirm、会话隐私和业务事实边界不变。详见 `docs/T46/ProjectFlow_Agent_Evaluation_Lab_Spec.md`、Slice 0/1 handoff 和 `docs/handoff.md`。
