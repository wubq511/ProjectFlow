...
│   │   └── utils/           # 工具函数
│   ├── skills/              # 7 SKILL.md files (project-intake/planning/task-breakdown/assignment/risk-replan/status/risk-analysis)
...
Phase 0-39 全部已完成（2026-05-28 ~ 2026-06-07）。Phase 40（2026-06-07）为 Agent sidebar UI 打磨阶段。Phase 41（2026-06-08）为全面安全审查与性能优化。T41 Agent Runtime 架构方案已在 2026-07-04 确认并提交，T41 runtime/tool slices 已实现到 S16。T42 ProjectMemory V1 issues #71-#80 已在 2026-07-07 合并；无剩余 V1 closure gap，PRD 可关闭。多模型多供应商配置与切换已在 2026-07-08 实现（commit `6f3e764`）：ModelConfigStore + DotEnvWriter + FileWatcher + ModelRouter registry + 动态 provider import + sidecar config API + 前端 settings dialog + agent sidebar 模型选择器。2026-07-09 全局输出本地化与工具可靠性修复：ID→Name 对照表注入系统提示（替代 annotateIdsForDisplay 双字段方案）、translateStatusValues 扩展至 55 个枚举值、translateFieldNames 补全、recentMessages 纳入 transformForLLM 管线、前端 translateStatus 共享翻译表、MultilineText 全局覆盖 + splitProseLines 增强分行；`analyze_checkins_and_risks` 支持单侧输入、task-breakdown `stage_id` 必填、协商接受/拒接 API + UI、tool failure 不再抛异常（返回 error content 让 LLM 重试）、agent_end 有 side effect 时不标 failed、新建 risk-analysis skill（`create_risk` 直接创建）、`replan-diff.tsx` ID 字段隐藏 + 枚举翻译、所有下拉框 SelectValue→直接 span 防 ID 泄漏。2026-07-10 PR #84/#85/#86 合并 + 对抗性审查修复：SSE 流式传输修复（currentEvent 作用域、ENDPOINT_EVENT_TYPE_MAP、轮询超时竞态、timeline 验证窗口、ConnectError fallback 持久化、降级成功语义）；Pi runtime 终态守卫（所有 tool_execution_* 事件 + message/turn 事件拒绝终态后写入）；app.ts err:unknown 类型窄化；确定性意图匹配器（sidecar 不可用 fallback）；`transformForLLM` 管线重构：`translateStatusValues` 盲字符串替换→`translateStatusValuesOnParsed` tree-walk（只替换叶子字符串值，永不触碰 key），`buildIdMappingTable` walk 递归所有值（修复 no-op），`FIELD_NAME_MAP` key 碰撞消歧；阶段进度条修复（activeStages.filter(completed)→completedStages.length/stages.length）。

> 当前阶段：Agent 输出质量与可靠性持续优化中。详见 docs/handoff.md。2026-07-10 对话输出通道修复：`agent.delta` 仅累积 `text_delta`（`toolcall_delta` 不进入可见文本），`agent_end` 从最后 assistant message 提取 `final_content`（替代 `accumulatedContent`），`message_update` 不再传递 `assistantMessageEvent`（消除 DB 膨胀），`execution_steps` 从 tool 生命周期事件收集并持久化到 `structured_payload`，前端移除 `turnCountRef`/`turnTokensRef`/`lastThinkingContent` 启发式并改用 `structured_payload.execution_steps` 折叠区。
