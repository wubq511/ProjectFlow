...
API key 必须放 `.env`，不能提交 Git。前端不能直接调用 LLM API。`LLM_PROVIDER` 默认 `mock`，真实 LLM 用 `openai` 或 `openai-compatible`。`LLM_TIMEOUT_SECONDS` 默认 `30.0`。`NEXT_PUBLIC_API_BASE_URL` 是前端可选变量，不配置时默认 `http://localhost:8000/api`。
...
| 15 | MVP Usable #19: Polish Project Intake, Workspace, and Member Profile UX | 已完成：2026-05-30 |
