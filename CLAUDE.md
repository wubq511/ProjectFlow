...
│   └── tests/unit/          # 58 test files, 1142 tests
├── frontend/
│   ├── src/
...
T44/T45 之后的 2026-07-14 UX/Runtime 打磨：设置入口从项目仪表盘右上角移至左侧栏底部；移除 AgentSidebar 右下角的”最近活动”区块；将”重置演示数据”迁入 SettingsDialog 的”系统”标签页，并重构设置页为标签页导航（模型配置 / 系统）与响应式布局；模型配置界面增加状态标签、操作下拉菜单与 API Key 设置优化。同时实现斜杠命令选中状态高亮 chip（SlashCommandChip）、消息气泡与输入框对齐、Composer 与 loop-boundary steering 消费；agent-bridge 新增 steering poller 与中流 abort 脚手架，后端/agent-bridge/前端 schema 与测试同步更新。当前验收基线更新为 852 backend passed / 4 skipped，1142 agent-bridge passed（58 files），191 frontend passed（18 files），三端 lint/typecheck/build 通过。2026-07-16 演示种子数据 V2 全面优化：11 任务、8 Agent 事件（含 negotiate+replan+rejected）、7 条 ProjectMemory（全覆盖 V1 类型+隐私边界）、5 AgentProposal（含 rejected lifecycle）、3 团队会话 18 消息、协商记录、状态更新轨迹、签到时间分散、时间线对齐真实开发节点。前端会话列表按月折叠、聊天消息折叠旧消息。方向卡/记忆/风险/事件去答辩化。详 `docs/demo-script.md` 和 `backend/app/seed/demo_projectflow.py`。

> 当前阶段：T44/T45 与重复生产 canary 已完成；2026-07-14 完成设置入口迁移、AgentSidebar 精简、设置页重构、斜杠命令高亮 chip 与 Composer steering 等 UX/Runtime 打磨。Flash 保持默认，Pro 只作显式质量升级；下一步可独立评估结构化成员约束和更紧凑的 workspace read view，不扩大 Proposal-Confirm、会话隐私或业务事实边界。详见 `docs/T44/agent-efficiency-model-config-spec.md`、`docs/T44/post-t44-production-canary-2026-07-13.md`、`docs/T45/agent-conversation-history-spec.md` 和 `docs/handoff.md`。
